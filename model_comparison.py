...
def get_test_params_from_yaml(test_suite):
    ...
    random_input_len = test_suite_params.get("random-input-len", [10240])
    random_output_len = test_suite_params.get("random-output-len", [256])
    max_concurrency = test_suite_params.get("max-concurrency", [1])
    
    test_configs = set()
    for np in num_prompts:
        for ni in random_input_len:
            for no in random_output_len:
                config = f"{np}-i{ni}-o{no}"
                test_configs.add(config)
    
    return {
        'test_configs': sorted(test_configs, key=lambda x: int(extract_concurrency_from_config(x) or 0)),
...
def get_model_data(chip, model_name, test_suite, run_id, config_with_concurrency):
    ...
    model_dir = os.path.join(reports_base, chip, "benchmark", model_name, test_suite, run_id)
    ...
    print(f"      Looking for config_path: {config_path}, exists: {os.path.isdir(config_path)}")
...
def generate_comparison_markdown(models_data, test_config, output_dir, chip, test_suite, run_ids, concurrency_list, concurrency):
...
def main():
    parser = argparse.ArgumentParser(description="Compare model performance across different models")
    parser.add_argument("--chip", type=str, required=True, help="Chip platform (e.g., hygon_bw1000, nvidia_h100)")
    parser.add_argument("--model", type=str, required=True, help="Model names to compare, separated by comma (e.g., MiniMax-M2.5-bf16,Qwen3.5-397B-A17B)")
    parser.add_argument("--test-suite", type=str, default="test_01", help="Test suite name (e.g., test_01)")
    parser.add_argument("--run-id", type=str, default="01", help="Run IDs for each model, separated by comma (e.g., 01 or 01,02)")
    ...
    if run_ids is None:
        print(f"Error: Invalid run-id format. Please provide either a single value or comma-separated values for {num_models} models")
        return
    
    if len(run_ids) != num_models:
        print(f"Error: Number of run-ids ({len(run_ids)}) does not match number of models ({num_models})")
        return
    ...
    for test_config in test_configs:
        ...
        for concurrency in concurrency_list:
            ...
            print(f"    Looking for: {chip}/benchmark/{models[0]}/{test_suite}/{run_ids[0]}/{config_with_concurrency}")
            ...
            for i, model_name in enumerate(models):
                rid = run_ids[i]
                model_dir = os.path.join("reports", chip, "benchmark", model_name, test_suite, rid)
                config_path = os.path.join(model_dir, config_with_concurrency)
                print(f"    Checking path: {config_path}")
                
                metrics = get_model_data(chip, model_name, test_suite, rid, config_with_concurrency)
                if metrics:
                    normalized_metrics = {}
                    for key, value in metrics.items():
                        normalized_metrics[key.lower()] = value
                    models_data[model_name] = normalized_metrics
                    print(f"    - {model_name} (run-id: {rid}): ✅ Loaded")
                else:
            ...
            generate_comparison_csv(models_data, config_with_concurrency, output_dir, concurrency)
            generate_comparison_charts(models_data, config_with_concurrency, output_dir, concurrency)
            generate_comparison_markdown(models_data, config_with_concurrency, output_dir, chip, test_suite, run_ids, concurrency_list, concurrency)
...
def generate_summary_report(output_base, test_suite, generated_reports):
    ...
    for config_key, concurrency in sorted_reports:
        ...
        summary_lines.append(f"- [{config_key} (CSV)]({csv_link}) | [Markdown]({md_link})")
