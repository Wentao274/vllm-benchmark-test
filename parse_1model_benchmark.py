...
def get_gpu_and_model_names(config_path):
    yaml_config = load_yaml_config(config_path)
    models_config = yaml_config.get("models", {})
    
    gpu_model_map = {}
    for gpu_name, model_list in models_config.items():
        for model_entry in model_list:
            model_name = model_entry.get("name")
            if model_name:
                if gpu_name not in gpu_model_map:
                    gpu_model_map[gpu_name] = []
                gpu_model_map[gpu_name].append(model_name)
    
    return gpu_model_map


def extract_test_id_from_path(log_file_path):
    parts = log_file_path.split(os.sep)
    for part in parts:
        if re.match(r'^test_\d+$', part):
            return part
    return None


def process_model(model_name, gpu_name, reports_base_dir, config_dir):
    ...
    model_reports_dir = os.path.join(reports_base_dir, gpu_name, model_name)
    
    results_by_test_id_config = defaultdict(lambda: defaultdict(dict))
    ...
    for log_file in log_files:
        ...
        test_id = extract_test_id_from_path(log_file)
        ...
        results_by_test_id_config[test_id][config_key][concurrency] = metrics
        
        print(f"[{gpu_name}/{model_name}] Parsed {test_id}/{num_prompts}-i{input_len}-o{output_len} - concurrency {concurrency}")
    
    for test_id, configs_data in results_by_test_id_config.items():
        for config_tuple, all_results in configs_data.items():
            ...
            output_dir = os.path.join("analysis", gpu_name, model_name, test_id, dir_name)
...
def main():
    ...
    gpu_model_map = get_gpu_and_model_names(yaml_path)
    
    if not gpu_model_map:
        print(f"No GPU configs found in {yaml_path}")
        return
    
    print(f"Found {len(gpu_model_map)} GPU(s): {', '.join(gpu_model_map.keys())}")
    
    Path("analysis").mkdir(exist_ok=True)
    
    for gpu_name, model_names in gpu_model_map.items():
        print(f"\n{'='*50}")
        print(f"Processing GPU: {gpu_name}")
        print(f"Models: {', '.join(model_names)}")
        print(f"{'='*50}")
        
        for model_name in model_names:
            print(f"\n--- Processing model: {model_name} ---")
            process_model(model_name, gpu_name, reports_base_dir, config_dir)
