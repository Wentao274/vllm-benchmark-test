...
| ----------- | ----------- | ----------- | ----------- | ----------- | ----------- | ----------- |
| 32 | 0.08 | 19.96 | 7097.50 | 259690.69 | 1140.06 | 1616.03 |
| 64 | 0.08 | 19.63 | 7103.03 | 666779.10 | 1139.86 | 1616.47 |
...
| 成功请求数 | 1000 | 1000 |
| 失败请求数 | 0 | 0 |
| 测试持续时间 (s) | 12716.29 | 12705.77 |
| 总输入 tokens | 90000000 | 90000000 |
| 总生成 tokens | 253835 | 249469 |
| **请求吞吐量 (req/s)** | 0.08 | 0.08 |
| **输出 token 吞吐量 (tok/s)** | 19.96 | 19.63 |
| 峰值输出 token 吞吐量 (tok/s) | 252.00 | 253.00 |
| 峰值并发请求数 | 35.00 | 67.00 |
| **总 token 吞吐量 (tok/s)** | 7097.50 | 7103.03 |
...
| 指标 | 32 并发 | 64 并发 |
|------|----------- | -----------|
| 平均 TTFT (ms) | 148188.94 | 542201.84 |
| 中位 TTFT (ms) | 144656.35 | 549301.21 |
| P95 TTFT (ms) | 186577.85 | 591795.52 |
| P99 TTFT (ms) | 259690.69 | 666779.10 |
...
| 指标 | 32 并发 | 64 并发 |
|------|----------- | -----------|
| 平均 TPOT (ms) | 1023.13 | 1040.36 |
| 中位 TPOT (ms) | 1040.08 | 1063.67 |
| P95 TPOT (ms) | 1134.86 | 1135.68 |
| P99 TPOT (ms) | 1140.06 | 1139.86 |
...
| 指标 | 32 并发 | 64 并发 |
|------|----------- | -----------|
| 平均 ITL (ms) | 1008.41 | 1021.00 |
| 中位 ITL (ms) | 1064.16 | 1073.12 |
| P95 ITL (ms) | 1568.60 | 1568.48 |
| P99 ITL (ms) | 1616.03 | 1616.47 |
...
**Token总吞吐量**: 最高达到 7103 tok/s (64 并发)。

### 2. 首Token延迟 (TTFT) 分析

TTFT随并发增加显著上升。
高并发平均 P99 TTFT: 666779ms；
最高 P99 TTFT 出现在 64 并发，达到 666779ms。

### 3. Token生成时间 (TPOT) 分析

TPOT随并发增加也呈上升趋势。
高并发平均 P99 TPOT: 1139.86ms；
最高 P99 TPOT 出现在 32 并发，达到 1140.06ms。

### 4. Token间延迟 (ITL) 分析

ITL随并发增加呈上升趋势。
高并发平均 P99 ITL: 1616.47ms；
最高 P99 ITL 出现在 64 并发，达到 1616.47ms。
