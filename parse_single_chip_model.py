...
def generate_comparison_charts(chip_data, concurrencies, output_dir, chip_name):
    ...
    fig.suptitle(f'{MODEL_NAME} on {chip_name} - Concurrency Comparison', fontsize=14, fontweight='bold')
    ...
    for i, v in enumerate(req_throughput):
        axes[0, 0].text(i, v + 0.02 * max(req_throughput) if max(req_throughput) > 0 else 0.1, f'{v:.2f}', ha='center', va='bottom', fontsize=9)
    ...
    for i, v in enumerate(output_tput):
        axes[0, 1].text(i, v + 0.02 * max(output_tput) if max(output_tput) > 0 else 1, f'{v:.0f}', ha='center', va='bottom', fontsize=9)
    ...
    for i, v in enumerate(total_tput):
        axes[0, 2].text(i, v + 0.02 * max(total_tput) if max(total_tput) > 0 else 100, f'{v:.0f}', ha='center', va='bottom', fontsize=9)
    ...
    for i, v in enumerate(ttft_p99):
        axes[1, 0].text(i, v + 0.02 * max(ttft_p99) if max(ttft_p99) > 0 else 10, f'{v:.0f}', ha='center', va='bottom', fontsize=9)
    ...
    for i, v in enumerate(tpot_p99):
        axes[1, 1].text(i, v + 0.02 * max(tpot_p99) if max(tpot_p99) > 0 else 1, f'{v:.2f}', ha='center', va='bottom', fontsize=9)
    ...
    for i, v in enumerate(itl_p99):
        axes[1, 2].text(i, v + 0.02 * max(itl_p99) if max(itl_p99) > 0 else 1, f'{v:.2f}', ha='center', va='bottom', fontsize=9)
...
def generate_performance_trends(chip_data, concurrencies, output_dir, chip_name):
    if not HAS_MATPLOTLIB:
        return None
    ...
    fig, axes = plt.subplots(2, 3, figsize=(15, 10))
    fig.suptitle(f'{MODEL_NAME} on {chip_name} - Performance Trends by Concurrency', fontsize=14, fontweight='bold')
    ...
    axes[0, 0].plot(concurrencies_int, values, '-o', color=colors[0], linewidth=2, markersize=6, label='QPS')
    ...
    axes[0, 1].plot(concurrencies_int, values, '-o', color=colors[1], linewidth=2, markersize=6, label='Output')
    ...
    axes[0, 2].plot(concurrencies_int, values, '-o', color=colors[2], linewidth=2, markersize=6, label='Total')
    ...
    axes[1, 0].plot(concurrencies_int, values_mean, '-o', color=colors[0], linewidth=2, markersize=6, label='Mean')
    axes[1, 0].plot(concurrencies_int, values_p99, '--s', color=colors[1], linewidth=1, markersize=4, alpha=0.6, label='P99')
    ...
    axes[1, 1].plot(concurrencies_int, values_mean, '-o', color=colors[0], linewidth=2, markersize=6, label='Mean')
    axes[1, 1].plot(concurrencies_int, values_p99, '--s', color=colors[1], linewidth=1, markersize=4, alpha=0.6, label='P99')
    ...
    axes[1, 2].plot(concurrencies_int, values_mean, '-o', color=colors[0], linewidth=2, markersize=6, label='Mean')
    axes[1, 2].plot(concurrencies_int, values_p99, '--s', color=colors[1], linewidth=1, markersize=4, alpha=0.6, label='P99')
...
def generate_analysis_content(chip_data, chip_name, concurrencies):
    ...
    throughputs = [float(chip_data.get(chip_name, {}).get(c, {}).get('Request throughput (req/s)', 0) or 0) for c in concurrencies]
    ...
    avg_high_tp = get_avg_by_conc_range('Request throughput (req/s)', high_conc) if high_conc else 0
    ...
    if low_conc:
        analysis_lines.append(f"低并发({','.join(low_conc)})平均 QPS: {avg_low_tp:.2f} req/s；")
    if mid_conc:
        analysis_lines.append(f"中并发({','.join(mid_conc)})平均 QPS: {avg_mid_tp:.2f} req/s；")
    if high_conc:
        analysis_lines.append(f"高并发({','.join(high_conc)})平均 QPS: {avg_high_tp:.2f} req/s；")
    analysis_lines.append(f"最高 QPS 出现在 {max_tp_conc} 并发，达到 {max_tp:.2f} req/s。\n")
    
    total_tput, total_tput_conc = get_max_value('Total token throughput (tok/s)')
    analysis_lines.append(f"**Token总吞吐量**: 最高达到 {total_tput:.0f} tok/s ({total_tput_conc} 并发)。\n")
    ...
    ttft_avg_high = get_avg_by_conc_range('P99 TTFT (ms)', high_conc) if high_conc else 0
    ...
    analysis_lines.append(f"最高 P99 TTFT 出现在 {ttft_p99_conc} 并发，达到 {ttft_p99:.0f}ms。\n")
    
    tpot_p99, tpot_p99_conc = get_max_value('P99 TPOT (ms)')
    tpot_avg_low = get_avg_by_conc_range('P99 TPOT (ms)', low_conc)
    tpot_avg_high = get_avg_by_conc_range('P99 TPOT (ms)', high_conc) if high_conc else 0
    ...
    analysis_lines.append(f"最高 P99 TPOT 出现在 {tpot_p99_conc} 并发，达到 {tpot_p99:.2f}ms。\n")
    ...
    analysis_lines.append(f"最高 P99 ITL 出现在 {itl_p99_conc} 并发，达到 {itl_p99:.2f}ms。\n")
    
    analysis_lines.append("### 5. 综合评估\n")
    if throughputs:
        growth_rate = (throughputs[-1] / throughputs[0] - 1) * 100 if throughputs[0] > 0 else 0
        analysis_lines.append(f"**吞吐量增长**: 从最低并发到最高并发，QPS增长了 {growth_rate:.1f}%。")
    
    if ttft_p99 > 0 and ttft_avg_low > 0:
        ttft_growth = (ttft_p99 / ttft_avg_low - 1) * 100 if ttft_avg_low > 0 else 0
        analysis_lines.append(f"**TTFT延迟恶化**: 高并发相比低并发，TTFT P99增加了 {ttft_growth:.1f}%。")
    
    if tpot_p99 > 0 and tpot_avg_low > 0:
        tpot_growth = (tpot_p99 / tpot_avg_low - 1) * 100 if tpot_avg_low > 0 else 0
        analysis_lines.append(f"**TPOT延迟恶化**: 高并发相比低并发，TPOT P99增加了 {tpot_growth:.1f}%。")
...
def generate_markdown_report(chip_data, concurrencies, output_dir, test_suite, chip_name,
    ...
    serving_table = "\n".join([
        f"| {name} | {make_table_for_metric(key)} |"
    ...
    concurrency_comparison_img = '<img src="./concurrency_comparison.png" width="1000" />'
    ...
    for param in param_names:
        ...
        param_rows.append(f"| {param} | {val} |")
    vllm_table = "\n".join(param_rows)
    
    remark = vllm_cfg.get("remarks", "")
    remarks_section = f"- **{chip_name}**: {remark}" if remark else ""
    
    md_content = f"""# {MODEL_NAME}模型在{chip_name}上的Benchmark基准测试报告
...
"""
    ...
    md_file = os.path.join(output_dir, f"{MODEL_NAME}_{chip_name}_concurrency.md")
