...
def get_test_overview(test_suite):
    ...
    if params:
        return {
            ...
            "input_context_length": params.get("random-input-len", []),
            "output_context_length": params.get("random-output-len", []),
        }
    return {
        "dataset": "random",
        "concurrency": [],
...
def generate_comparison_csv(runid_data, concurrencies, output_dir, chip_name):
    ...
    for display_name, key_name in metric_names:
        ...
        for conc in concurrencies:
            val1 = runid_data.get(run_id1, {}).get(chip_name, {}).get(conc, {}).get(key_name, "")
            val2 = runid_data.get(run_id2, {}).get(chip_name, {}).get(conc, {}).get(key_name, "")
...
def generate_comparison_charts(runid_data, concurrencies, output_dir, chip_name):
    ...
    def get_values(run_id, key):
        values = []
        for conc in concurrencies:
            val = runid_data.get(run_id, {}).get(chip_name, {}).get(conc, {}).get(key, "0")
            try:
    ...
    fig.suptitle(f'{MODEL_NAME} on {chip_name} - Run ID Comparison ({run_id1} vs {run_id2})', fontsize=14, fontweight='bold')
    
    req_throughput_1 = get_values(run_id1, 'Request throughput (req/s)')
    req_throughput_2 = get_values(run_id2, 'Request throughput (req/s)')
    
    axes[0, 0].bar([i - width/2 for i in x], req_throughput_1, width, label=run_id1, color=colors[0], alpha=0.8)
    axes[0, 0].bar([i + width/2 for i in x], req_throughput_2, width, label=run_id2, color=colors[1], alpha=0.8)
    ...
    axes[0, 1].bar([i - width/2 for i in x], output_tput_1, width, label=run_id1, color=colors[0], alpha=0.8)
    axes[0, 1].bar([i + width/2 for i in x], output_tput_2, width, label=run_id2, color=colors[1], alpha=0.8)
    ...
    axes[0, 2].bar([i - width/2 for i in x], total_tput_1, width, label=run_id1, color=colors[0], alpha=0.8)
    axes[0, 2].bar([i + width/2 for i in x], total_tput_2, width, label=run_id2, color=colors[1], alpha=0.8)
    ...
    axes[1, 0].bar([i - width/2 for i in x], ttft_p99_1, width, label=run_id1, color=colors[0], alpha=0.8)
    axes[1, 0].bar([i + width/2 for i in x], ttft_p99_2, width, label=run_id2, color=colors[1], alpha=0.8)
    ...
    axes[1, 1].bar([i - width/2 for i in x], tpot_p99_1, width, label=run_id1, color=colors[0], alpha=0.8)
    axes[1, 1].bar([i + width/2 for i in x], tpot_p99_2, width, label=run_id2, color=colors[1], alpha=0.8)
    ...
    axes[1, 2].bar([i - width/2 for i in x], itl_p99_1, width, label=run_id1, color=colors[0], alpha=0.8)
    axes[1, 2].bar([i + width/2 for i in x], itl_p99_2, width, label=run_id2, color=colors[1], alpha=0.8)
...
def generate_markdown_report(runid_data, concurrencies, output_dir, test_suite, chip_name,
                              vllm_config=None, test_overview=None):
...
"""
    ...
    if tp_improvement > 0:
        analysis_lines.append(f"**请求吞吐量**: RUN-{run_id2} 相比 RUN-{run_id1} 平均提升 **{tp_improvement:.1f}%**\n")
    else:
        analysis_lines.append(f"**请求吞吐量**: RUN-{run_id2} 相比 RUN-{run_id1} 平均变化 **{tp_improvement:.1f}%**\n")
    
    if output_tp_improvement > 0:
        analysis_lines.append(f"**输出Token吞吐量**: RUN-{run_id2} 相比 RUN-{run_id1} 平均提升 **{output_tp_improvement:.1f}%**\n")
    else:
        analysis_lines.append(f"**输出Token吞吐量**: RUN-{run_id2} 相比 RUN-{run_id1} 平均变化 **{output_tp_improvement:.1f}%**\n")
    
    analysis_lines.append("### 延迟对比\n")
    if ttft_change > 0:
        analysis_lines.append(f"**TTFT P99**: RUN-{run_id2} 相比 RUN-{run_id1} 平均增加 **{ttft_change:.1f}%** (延迟增加)")
    else:
        analysis_lines.append(f"**TTFT P99**: RUN-{run_id2} 相比 RUN-{run_id1} 平均改善 **{abs(ttft_change):.1f}%** (延迟降低)")
    
    if tpot_change > 0:
        analysis_lines.append(f"**TPOT P99**: RUN-{run_id2} 相比 RUN-{run_id1} 平均增加 **{tpot_change:.1f}%** (延迟增加)")
    else:
        analysis_lines.append(f"**TPOT P99**: RUN-{run_id2} 相比 RUN-{run_id1} 平均改善 **{abs(tpot_change):.1f}%** (延迟降低)")
    
    if itl_change > 0:
        analysis_lines.append(f"**ITL P99**: RUN-{run_id2} 相比 RUN-{run_id1} 平均增加 **{itl_change:.1f}%** (延迟增加)")
    else:
        analysis_lines.append(f"**ITL P99**: RUN-{run_id2} 相比 RUN-{run_id1} 平均改善 **{abs(itl_change):.1f}%** (延迟降低)")
    ...
    vllm_table_rows = []
    for param, values in vllm_config.items():
        if isinstance(values, dict):
            val1 = values.get(run_id1, 'N/A')
            val2 = values.get(run_id2, 'N/A')
        else:
            val1 = values
            val2 = values
        vllm_table_rows.append(f"| {param} | {val1} | {val2} |")
    vllm_table = "\n".join(vllm_table_rows) if vllm_table_rows else "| 参数 | - | - |"
        ...
        for chip in chip_configs:
            ...
            for run_id in RUN_IDS:
                if not common_concurrencies:
                    ...
                    common_concurrencies = common_concurrencies.intersection(run_id_concurrencies[run_id])
            ...
            print(f"Found {len(concurrencies)} common concurrency levels: {', '.join(concurrencies)}")
            ...
            if HAS_MATPLOTLIB:
                generate_comparison_charts(runid_data, concurrencies, output_base, chip_name)
            
            generate_markdown_report(runid_data, concurrencies, output_base, test_suite, chip_name,
                                     vllm_config=VLLM_CONFIG, test_overview=test_overview)
            
            print(f"\n{'='*50}")
            print(f"Run ID comparison for {chip_name} - {test_suite} generated successfully!")
