...
# 测试综述
### 下表列出各测试场景下，所测芯片间的性能比对结果
| 测试场景                | 请求参数                     | 请求并发数                                   | 性能比对                 | 备注  |
|---------------------|--------------------------|-----------------------------------------|----------------------|-----|
| vllm bench基准测试      | input: 10k, output:0.25k | [ 1, 2, 4, 8, 10, 16, 32, 64, 80, 128 ] | H100 > P800 > BW1000 |     |
| 单、多并发超长上下文请求        | input: 190k, output: 1k  | [ 1, 2, 4, 8, 10 ]                      | H100 > P800 > BW1000 |     |
| 多轮对话                | requests/s               | 请求吞吐                                    |                      |     |
...
## 测试场景二：单并发超长上下文请求测试
**测试目标**：获取单并发超长上下文请求（接近启动参数的max-len 192k）下个芯片的性能指标进行比对。
