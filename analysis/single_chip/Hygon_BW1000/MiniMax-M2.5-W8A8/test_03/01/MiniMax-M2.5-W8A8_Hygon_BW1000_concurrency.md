...
| ----------- | ----------- | ----------- | ----------- | ----------- | ----------- | ----------- |
| 32 | 0.03 | 58.42 | 2687.12 | 840076.37 | 164.03 | 2600.66 |
| 64 | 0.03 | 58.35 | 2683.97 | 1925575.70 | 164.14 | 2600.66 |
...
| 成功请求数 | 1000 | 1000 |
| 失败请求数 | 0 | 0 |
| 测试持续时间 (s) | 34237.41 | 34277.62 |
| 总输入 tokens | 90000000 | 90000000 |
| 总生成 tokens | 2000000 | 2000000 |
| **请求吞吐量 (req/s)** | 0.03 | 0.03 |
| **输出 token 吞吐量 (tok/s)** | 58.42 | 58.35 |
| 峰值输出 token 吞吐量 (tok/s) | 270.00 | 260.00 |
| 峰值并发请求数 | 33.00 | 65.00 |
| **总 token 吞吐量 (tok/s)** | 2687.12 | 2683.97 |
...
| 指标 | 32 并发 | 64 并发 |
|------|----------- | -----------|
| 平均 TTFT (ms) | 763172.93 | 1819207.46 |
| 中位 TTFT (ms) | 758207.30 | 1843948.68 |
| P95 TTFT (ms) | 838778.34 | 1923340.65 |
| P99 TTFT (ms) | 840076.37 | 1925575.70 |
...
| 指标 | 32 并发 | 64 并发 |
|------|----------- | -----------|
| 平均 TPOT (ms) | 161.97 | 162.03 |
| 中位 TPOT (ms) | 162.90 | 162.94 |
| P95 TPOT (ms) | 163.67 | 163.72 |
| P99 TPOT (ms) | 164.03 | 164.14 |
...
| 指标 | 32 并发 | 64 并发 |
|------|----------- | -----------|
| 平均 ITL (ms) | 161.96 | 162.02 |
| 中位 ITL (ms) | 41.24 | 41.29 |
| P95 ITL (ms) | 56.02 | 55.61 |
| P99 ITL (ms) | 2600.66 | 2600.66 |
...
**Token总吞吐量**: 最高达到 2687 tok/s (32 并发)。

### 2. 首Token延迟 (TTFT) 分析

TTFT随并发增加显著上升。
高并发平均 P99 TTFT: 1925576ms；
最高 P99 TTFT 出现在 64 并发，达到 1925576ms。

### 3. Token生成时间 (TPOT) 分析

TPOT随并发增加也呈上升趋势。
高并发平均 P99 TPOT: 164.14ms；
最高 P99 TPOT 出现在 64 并发，达到 164.14ms。

### 4. Token间延迟 (ITL) 分析

ITL随并发增加呈上升趋势。
高并发平均 P99 ITL: 2600.66ms；
最高 P99 ITL 出现在 32 并发，达到 2600.66ms。
